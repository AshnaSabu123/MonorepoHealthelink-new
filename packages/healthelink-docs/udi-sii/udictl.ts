...
const duckDbDDL = (config = configure()) => {
  const { excel: { SCF }, sqlite: { PRIME } } = config.integrations;
...
const emitIngestSh = (options: {
  ...
  if (options.destroyExisting) {
    ...
    ingestDuckDbCmds.push(
      `rm -f ${
        config.integrations.sqlite.PRIME.sqliteDbFsPath(config.emitCtx)
      }`,
    );
  }
  ...
  ingestDuckDbCmds.push(
    `deno run -A ${
      config.baseFsPath(path.basename(import.meta.url))
...
function generateGovnDocsSh(config = configure()) {
  ...
  return `cd ${gsda} && find . -name "*.tbls.yaml" -exec ${
    path.relative(gsda, config.binFsPath("tbls"))
  } doc --rm-dist --config {} \\;`;
}
