function markdownGenerator(rootPath: string, dirsCreated: Record<string, {
    readonly fsPath: string;
    readonly humanPath: string
}>) {
    return async (humanPath: string) => {
        const humanPathParts = humanPath.split('/');
        const fsPathParts = humanPath.split('/').map(part => part.toLowerCase().replaceAll(' & ', ' and ').split(' ').join('-'));
        const fsPath = fsPathParts.slice(0, -1).join('/');
        const filename = `${fsPathParts[fsPathParts.length - 1]}.md`;
        ...
        if (!dirsCreated[fsPath]) {
            ...
            dirsCreated[fsPath] = { fsPath: mkPath, humanPath: humanPathParts.slice(0, -1).join('/') };
        }

        const content = `---\ntitle: ${humanPathParts[humanPathParts.length - 1]}\n---\n\n# TODO`;
        try {
...
await generateMarkdown("Code Quality and Repositories/Collaboration Tools for Developers");
await generateMarkdown("Code Quality and Repositories/Environment Setup");

await generateMarkdown("Teams and Roles/Product Management/Responsibilities & Duties");
await generateMarkdown("Teams and Roles/Product Management/Team Members & Contacts");
await generateMarkdown("Teams and Roles/Product Management/Collaboration with Other Teams");
await generateMarkdown("Teams and Roles/Product Management/Key Deliverables & Timeline");
...
await generateMarkdown("Teams and Roles/Development/Collaboration Tools & Platforms");
...
await generateMarkdown("Teams and Roles/Development/Feedback & Continuous Improvement");
await generateMarkdown("Teams and Roles/QA and Testing/Responsibilities & Duties");
await generateMarkdown("Teams and Roles/QA and Testing/Team Members & Contacts");
await generateMarkdown("Teams and Roles/QA and Testing/Collaboration with Development");
await generateMarkdown("Teams and Roles/QA and Testing/Tools & Platforms Used");
await generateMarkdown("Teams and Roles/QA and Testing/Test Planning & Execution");
await generateMarkdown("Teams and Roles/Operations/Ops Team Structure & Responsibilities");
await generateMarkdown("Teams and Roles/Operations/Incident Management Protocols");
await generateMarkdown("Teams and Roles/Operations/Infrastructure Management");
await generateMarkdown("Teams and Roles/Operations/Security Protocols & Best Practices");
await generateMarkdown("Teams and Roles/Sales & Marketing/Responsibilities & Duties");
await generateMarkdown("Teams and Roles/Sales & Marketing/Team Members & Contacts");
await generateMarkdown("Teams and Roles/Sales & Marketing/Product Demonstration & Training");
await generateMarkdown("Teams and Roles/Sales & Marketing/Market Research & Feedback Collection");
await generateMarkdown("Teams and Roles/Other Affiliated Professionals/Responsibilities & Duties");
await generateMarkdown("Teams and Roles/Other Affiliated Professionals/Team Members & Contacts");
await generateMarkdown("Teams and Roles/Other Affiliated Professionals/Collaboration Guidelines");
await generateMarkdown("Teams and Roles/Other Affiliated Professionals/Key Deliverables & Timeline");

await generateMarkdown("Collaboration & Communication/Collaboration Tools & Platforms");
